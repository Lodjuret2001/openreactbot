...
const OpenReactBot = ({ API_KEY, AIConfig, stylesConfig, }) => {
    ...
    const config = createRckConfig(styles.name, styles.headerText, styles.chatBotImg, styles.userImg, AIConfig, AIConfig.startMessage, openai);
    return (<>
      {isOpen ? (<ChatBotMainContainer>
          <CloseChatBot handleSetIsOpen={handleSetIsOpen}/>
          <Chatbot config={config} actionProvider={ActionProvider} messageParser={MessageParser}/>
        </ChatBotMainContainer>) : (<LogoButton styles={styles} handleSetIsOpen={handleSetIsOpen}/>)}
    </>);
};
