...
const OpenReactBot = ({ API_KEY, AIConfig, stylesConfig, }) => {
    ...
    const { chatBotMessages, setChatbotMessages, isTyping, setInput, setIsInitilalized, } = useOpenAI(API_KEY, AIConfig);
    ...
    return (<>
      {isOpen ? (<ChatBotContainer styles={styles}>
          <CloseChatBot handleSetIsOpen={handleSetIsOpen}/>
          <MainContainer>
            <ChatContainer>
              <MessageList scrollBehavior="smooth" typingIndicator={isTyping ? (<TypingIndicator className=".orb-typing--indicator" content={`${styles.name} is typing...`}/>) : undefined}>
                ...
                return (<Message key={i} model={{
                      ...
                      <Avatar key={i} src={chat.role === "assistant"
                        ? styles.chatBotImg
                        : styles.userImg}/>
                    </Message>);
            })}
              </MessageList>
              <MessageInput placeholder={styles.placeholder} onSend={handleSend} autoFocus={true} attachButton={false}/>
            </ChatContainer>
          </MainContainer>
        </ChatBotContainer>) : (<LogoButton styles={styles} handleSetIsOpen={handleSetIsOpen}/>)}
    </>);
};
